/**
 * Companion to NexusCodingPanel.test.ts that exercises the panel with the
 ...
 * getSettings -> NexusCodingPanel._getSettings cache -> webview HTML.
 */
...
describe("NexusCodingPanel with real settings module", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("renders the webview HTML using a custom modelName read via real getSettings", () => {
    ...
    const panel = new NexusCodingPanel(makeExtensionUri(), new NexusCodingRuntime());
  ...
  it("reads getConfiguration with the gemma-code section", () => {
    ...
    new NexusCodingPanel(makeExtensionUri(), new NexusCodingRuntime());

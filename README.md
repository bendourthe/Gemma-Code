...
| v1.18.0 | Agent harness and governance: skill-native mappings, llama.cpp loopback recipe, live harness selector, catalog/registry governance, ask inbox + scheduler, ACP surface, OS process sandbox | In progress (Phase 7 close-out; Phase 4 skipped) | [docs/v1/v1.18/](docs/v1/v1.18/) |
...
| **Local API server** | Opt-in loopback OpenAI/Anthropic gateway (`nexus.serving.enabled`, default off) so other tools on this machine reuse installed models. ACP (`nexus.acp.enabled`) mounts `POST /acp` on the same listener and token. |

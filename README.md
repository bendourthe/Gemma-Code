...
The repository currently contains the Gemma Code engine at version 0.22.x. Until the v1.0.0 desktop-app shell lands, the user-facing surface is the VS Code extension.
...
For the Nexus pivot — the v1.0.0 desktop-app plan, module decomposition, comparisons against ComfyUI and DevAI-Hub, installer scope, and phased delivery — see [docs/v1.0.0/](docs/v1.0.0/).
...
The v1.0.0 cycle will introduce a `desktop/` (Electron / Tauri shell) and a `modules/` subtree (one folder per module: `coding/`, `chat/`, `image/`, `video/`), and reshape `src/` into the shared core. The detailed layout lands with the v1.0.0 plan.

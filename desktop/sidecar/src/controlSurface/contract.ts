 ...
 *   3. The v2.1.0 JSON CLI (`/nexus/*`) -- sibling of ACP, never under `/v1`.
 *
 ...
 *     route handler (serving, ACP, or JSON CLI) runs.
 ...
 *     both are off (no idle port). JSON CLI (`/nexus/*`) rides the same
 *     listener and the same `nexus.serving.token`; it is never mounted under `/v1`.
 ...
 *     not silently enable the other. JSON CLI routes are always registered.
 *
...
export const CONTROL_SURFACE_JSON_CLI_PREFIX = "/nexus";

/** Documented route families that may mount on the shared listener. */
export const CONTROL_SURFACE_MOUNTS = ["serving", "acp", "json-cli"] as const;

...
| Sandbox | Exec-sandbox audit exists (`docs/v1/v1.12/exec-sandbox-audit.md`); headless tool sandboxing in `modules/coding/runtime/headlessTools.ts`; no durable per-project sandbox | "its durable computer, where installed tools stay installed" -- per-scope isolated sandbox behind a fixed `execute` tool | Gap (A4) |
...
- **A4 -- Durable per-project exec sandbox.** A persistent, project-scoped sandbox where installed tooling survives across sessions ("durable computer"), building on the v1.12.0 exec-sandbox audit (`docs/v1/v1.12/exec-sandbox-audit.md`).
...
- **(Dedup) Crons and watches.** QM's background crons map to the local scheduler already adopted as A2 of the OpenWorker comparison (`docs/v1/v1.18/comparisons/v1.18.0-comparison-openworker.md`); no new candidate.
...
| A4 Durable per-project sandbox (P2) | QM durable per-scope sandbox | Project-keyed persistent exec sandbox per `docs/v1/v1.12/exec-sandbox-audit.md` | High | A1 | Medium |

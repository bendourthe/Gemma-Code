...
| v1.6.0 (companion) | OpenRouter Fusion technique reverse-engineered local-only (F1 fuse skill, F2 panel executor, F3 VRAM co-residency, F4 budget-panel routing + A/B) | **In progress (Phase 2 of 5)** -- Phase 1 (F1 `fuse` skill + `council` synthesis upgrade + schema-conformance test, OF001-OF003) landed 2026-06-16; Phase 2 (F2 `PanelExecutor`/`FusionAgent` distinct-model fan-out + F5 untrusted-input/tool-isolation hardening, OF004-OF006) landed 2026-06-16. Phases 3-5 (concurrent VRAM residency, routing heuristic + A/B, FINAL acceptance gate) pending | `versions/v1/v1.6.0/plans/adoption-openrouter-fusion.md` |

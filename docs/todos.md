...
| v1.6.0 (companion) | OpenRouter Fusion technique reverse-engineered local-only (F1 fuse skill, F2 panel executor, F3 VRAM co-residency, F4 budget-panel routing + A/B) | **In progress (Phase 1 of 5)** -- Phase 1 (F1 structured judge-fusion synthesis skill: `fuse` skill + `council` synthesis upgrade + schema-conformance test, OF001-OF003) landed 2026-06-16. Phases 2-5 (panel orchestration + eval-integrity, concurrent VRAM residency, routing heuristic + A/B, FINAL acceptance gate) pending | `versions/v1/v1.6.0/plans/adoption-openrouter-fusion.md` |

...
Version detected from `CHANGELOG.md` first heading (`## [0.6.0] -- 2026-05-04`), the in-flight v0.6.0 release entry. The `package.json` field still reads `0.5.5`; sub-task 8.5 of [docs/v0.6.0/plans/v0.6.0-cycle.md](./plans/v0.6.0-cycle.md) bumps it to match.
...
| Markdown | marked | ^4.3.0 | v12 migration deferred to v0.7.0 (per [docs/v0.6.0/development/history/2026-05_phase-7-polish.md](./development/history/2026-05_phase-7-polish.md) §7.5) |
...
- `marked` is held at v4 deliberately (the v12 `Renderer` API is incompatible with the three custom renderers in `MarkdownRenderer.ts`); migration is logged at [docs/v0.6.0/review/known-gaps.md](./review/known-gaps.md) §11.1.
...
- [x] Output path `docs/v0.6.0/analysis.md` resolves inside the project root.

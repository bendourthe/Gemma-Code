...
**Companion reports**: [comparison-agentmemory.md](comparison-agentmemory.md); upstream comparison artifacts in [docs/v1.0.0/comparison-comfyui.md](../v1.0.0/comparison-comfyui.md) and [docs/v1.0.0/comparison-devai-hub.md](../v1.0.0/comparison-devai-hub.md).
...
- **NVIDIA-only speed advantage**: SANA's linear-attention kernel is Triton-based; on macOS Apple Silicon it falls back to standard attention. The speed advantage drops to ~2x vs SDXL Turbo (still a win). Document this in `docs/v1.1.0/sana-platform-notes.md`.
...
- **Default model swap impact**: Changing the default from SDXL Turbo to SANA-1.6B changes the visual character of outputs. Document in [docs/v1.1.0/release-notes.md](release-notes.md) with side-by-side samples. SDXL Turbo remains in the catalog as an opt-in alternative.

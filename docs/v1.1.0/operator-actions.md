...
- **What**: Run `/run-deep-review` (which internally chains `analyze-codebase` + `review-codebase` + `run-security-audit` + `run-penetration-test --depth=deep`) against the v1.1.0 delta. Output under [docs/v1.1.0/review/](review/) mirroring the v1.0.0 layout. The static portion ships in this commit (`synthesis.md` Section 8); the live chain is operator-gated because the active host lacks the full DevAI-Hub skill harness wiring + the network egress + the headed display. Triage every finding: P0 / P1 -> close in this phase before the v1.1.0 tag pushes; P2 / P3 -> log into `docs/v1.1.0/known-gaps.md`.
- **Expected outputs**: `docs/v1.1.0/review/analyze-codebase.md`, `docs/v1.1.0/review/review-codebase.md`, `docs/v1.1.0/review/security-audit.md`, `docs/v1.1.0/review/penetration-test.md` (overlays the static `synthesis.md`).
...
- **Acceptance**: signed Windows installer; notarized macOS DMG; tested Linux AppImage; the three OA entries in `docs/v1.0.0/operator-actions.md` are signed off with v1.1.0 timestamps.
...
- **What**:
  ...
  2. **OA-09**: with the four resident diffusion models (~17 GB total), run timing benchmarks for SANA-1.6B / Sana-Sprint / 2K / 4K / int4 / SANA-Video / LTX-Video / SVD; commit timings to `docs/v1.1.0/operator-actions.md` and verify they meet Phase 12 / 13 stability gates within +/-10%.
  3. **OA-10**: run `nexus skills sync` against the live upstream DevAI-Hub, verify the resulting `~/.nexus/skills/devai-hub/<tag>/manifest.json`, commit a redacted log to `docs/v1.1.0/operator-actions.md`.
...
- **What**: On a Windows 11 fresh VM, a macOS Sequoia fresh VM, and an Ubuntu 24.04 fresh VM, run through each `docs/v1.1.0/installer-smoke-{windows,macos,linux}.md` checklist top-to-bottom. Record results in `docs/v1.1.0/installer-smoke-{windows,macos,linux}-rtm.md`.

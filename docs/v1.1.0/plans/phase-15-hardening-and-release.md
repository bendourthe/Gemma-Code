...
**Prompt**:
> Invoke `/run-deep-review` (or the manual equivalent: `analyze-codebase`, `review-codebase`, `run-security-audit`, `run-penetration-test --depth=deep`). Output under [docs/v1.1.0/review/](../review/) mirroring the v1.0.0 layout. Triage every finding: P0 / P1 -> close in this phase; P2 / P3 -> log into `docs/v1.1.0/known-gaps.md`. Acceptance: review artifacts committed; zero P0 / P1 open.
...
**Prompt**:
> On a Windows 11 fresh VM, a macOS Sequoia fresh VM, and an Ubuntu 24.04 fresh VM, run through each checklist top-to-bottom. Record results in `docs/v1.1.0/installer-smoke-{windows,macos,linux}-rtm.md`. Acceptance: every checklist signs off green; any partial-pass items either fix-and-re-test or open a tracked known-gap.
...
**Prompt**:
> The landing page work (`nexus.bendourthe.com/download` or equivalent) is logged as deferred in `docs/v1.0.0/known-gaps.md` 11.P2.OOO and rolls forward into v1.1.1 per [operator-actions.md](../../v1.0.0/operator-actions.md) OA-05. For v1.1.0 release distribution, the GitHub Release URL is canonical. Update [docs/v1.1.0/distribution.md](../distribution.md) (mirror the v1.0.0 file's structure) accordingly. Acceptance: distribution.md is committed; the GitHub Release page is populated with all three installer artifacts + SHA manifest.

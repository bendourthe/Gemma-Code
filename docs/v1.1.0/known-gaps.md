...
Phase 5 contributes seven closures (agentmemory A2 local embedder, A1 BM25 + dense + RRF + façade, the warm-build worker, and the benchmark + settings keys) and three new open items (5.1.P1.M `LocalEmbedder` runs against the deterministic hash fallback in CI until the Phase 14 installer payload lands, 5.5.P1.N the `src/storage/UnifiedMemoryRetriever` migration is deferred to Phase 6/9 because the substring path actually lives in `MemoryHub` and that's where the hybrid delegation went, 5.6.P2.O the `IdleTimeScheduler.register(createWarmRebuildTask(...))` call site in the sidecar is deferred until the Phase 6 `MemoryStore` adapters give the worker a real source). The Phase 4 closures stay on the books from the prior pass.

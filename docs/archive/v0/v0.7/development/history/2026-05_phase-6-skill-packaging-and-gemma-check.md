...
A new published `bin` (`gemma-check`) wraps a small rule set under `lib/checks/`. Each rule module exports the same three-symbol contract: `{ id, severity, scan(filePath, contents): Finding[] }`. The central registry is [lib/checks/index.mjs](../../../../versions/lib/checks/index.mjs), which exports `RULES` (array, controls report ordering) and `RULE_BY_ID` (index by id).
...
Two cross-cutting helpers live in [lib/checks/helpers.mjs](../../../../versions/lib/checks/helpers.mjs):
...
- [src/utils/secretPaths.ts](../../../../versions/src/utils/secretPaths.ts) line 20: the `"**/.env*"` entry in `SECRET_PATH_PATTERNS` -- a denylist that detects env files, so it has to reference the literal.
- [src/storage/MemoryHealthCheck.ts](../../../../versions/src/storage/MemoryHealthCheck.ts) line 19: the `SECRET_TOKEN_REGEX` that detects `.env.*` tokens -- same reason.
...
**[tests/unit/cli/gemma-check.test.ts](../../../../versions/tests/unit/cli/gemma-check.test.ts)** -- 60 cases organised into five layers:
...
**[tests/unit/scripts/package-skills.test.ts](../../../../versions/tests/unit/scripts/package-skills.test.ts)** -- 14 cases:

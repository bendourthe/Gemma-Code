...
**Date:** 2026-04-19
**Plan:** [docs/archive/versions/v0/v0.4.0/implementation-plan.md](../../implementation-plan.md) Phase 3
**Findings closed:** 24 (8 P1 + 10 P2 + 6 P3) from [docs/archive/versions/v0/v0.3.0/review.md](../../../v0.3/review.md)
...
- **File:** [src/safety/GitSafetyNet.ts:57-79](../../../../versions/src/safety/GitSafetyNet.ts#L57-L79)
- **Result:** DEVIATION: the inverted diff check appears to have been corrected in a prior phase. Added a regression test in [tests/unit/safety/GitSafetyNet.test.ts](../../../../versions/tests/unit/safety/GitSafetyNet.test.ts) that locks the correct behavior: two consecutive `commitAgentChanges` invocations produce exactly one commit when only the first has staged changes.

### 3.2 Remove double-confirmation for file-edit tools
- **Files:** [src/tools/ToolRegistry.ts](../../../../versions/src/tools/ToolRegistry.ts), [src/panels/GemmaCodePanel.ts](../../../../versions/src/panels/GemmaCodePanel.ts)
...
- **Tests:** 5 new cases in [tests/unit/tools/ToolRegistry.test.ts](../../../../versions/tests/unit/tools/ToolRegistry.test.ts) cover `delete_file` (central fires), `write_file` / `edit_file` in ask / plan (central skipped), `create_file` in auto (central fires), and user-rejection propagation.

### 3.3 Remove unregistered tools from catalog
- **File:** [src/tools/ToolCatalog.ts](../../../../versions/src/tools/ToolCatalog.ts)
- **Result:** `TOOL_CATALOG` shrinks from 13 to 10 entries. `BuiltinToolName` in [src/tools/types.ts](../../../../versions/src/tools/types.ts) still carries the three legacy names so Phase 7 can cleanly delete `LazyToolLoader` and `OutputRedirector`. [tests/unit/tools/LazyToolLoader.test.ts](../../../../versions/tests/unit/tools/LazyToolLoader.test.ts) builds a synthetic catalog that still contains the helpers.

### 3.4 Wire BudgetMiddleware.recordTurnTokens
- **Files:** [src/tools/AgentLoop.ts](../../../../versions/src/tools/AgentLoop.ts), [src/tools/BudgetMiddleware.ts](../../../../versions/src/tools/BudgetMiddleware.ts)
...
- **Tests:** Two integration tests in [tests/unit/tools/AgentLoop.test.ts](../../../../versions/tests/unit/tools/AgentLoop.test.ts) assert both halts fire.

### 3.5 Remove _budgetEnforcer branches from AgentLoop
- **File:** [src/tools/AgentLoop.ts](../../../../versions/src/tools/AgentLoop.ts)
...
- **File:** [src/chat/ConversationManager.ts](../../../../versions/src/chat/ConversationManager.ts)
...
- **New file:** [src/storage/embeddingUtils.ts](../../../../versions/src/storage/embeddingUtils.ts)
...
- **Tests:** 16 cases in [tests/unit/storage/embeddingUtils.test.ts](../../../../versions/tests/unit/storage/embeddingUtils.test.ts).

### 3.8 Extract FTS5 trigger helper
- **New file:** [src/storage/sqliteFts.ts](../../../../versions/src/storage/sqliteFts.ts)
...
- **File:** [src/tools/Gemma4ToolFormat.ts](../../../../versions/src/tools/Gemma4ToolFormat.ts)
...
- **Tests:** 4 new cases in [tests/unit/tools/Gemma4ToolFormat.test.ts](../../../../versions/tests/unit/tools/Gemma4ToolFormat.test.ts) covering object values, array values, deeply nested objects with strings containing braces, and malformed-JSON fallback.
...
- **File:** [src/chat/PromptBuilder.ts](../../../../versions/src/chat/PromptBuilder.ts)
...
- **New file:** [src/orchestration/ComplexityClassifier.ts](../../../../versions/src/orchestration/ComplexityClassifier.ts)
...
- **Tests:** 6 cases in [tests/unit/orchestration/ComplexityClassifier.test.ts](../../../../versions/tests/unit/orchestration/ComplexityClassifier.test.ts) covering simple-prefix / trigger-keyword / length-threshold / boundary / precedence.

### 3.13 Fix MemoryConsolidator unreachable user_requested branch
- **Files:** [src/storage/MemoryConsolidator.ts](../../../../versions/src/storage/MemoryConsolidator.ts), [src/storage/MemoryLayers.types.ts](../../../../versions/src/storage/MemoryLayers.types.ts)
...
- **Files:** [src/tools/handlers/filesystem.ts](../../../../versions/src/tools/handlers/filesystem.ts), [src/tools/types.ts](../../../../versions/src/tools/types.ts)
...
- **File:** [src/storage/EntityExtractor.ts](../../../../versions/src/storage/EntityExtractor.ts)
...
- **File:** [src/panels/TraceDashboardPanel.ts](../../../../versions/src/panels/TraceDashboardPanel.ts)
...
- **Item 4 (magic numbers):** [src/storage/constants.ts](../../../../versions/src/storage/constants.ts) created with `CHARS_PER_TOKEN`, `MAX_NODES_VISITED`, `GRAPH_MAX_TRAVERSAL_RESULTS`, `ONE_DAY_MS`, `ONE_WEEK_MS`; applied to `GraphQueryEngine` and `GraphMemory`.
...
- **File:** [src/config/HardwareTier.ts](../../../../versions/src/config/HardwareTier.ts)
- **Result:** Unused export gone; `ModelRecommendation` import removed; corresponding test block in [tests/unit/config/HardwareTier.test.ts](../../../../versions/tests/unit/config/HardwareTier.test.ts) deleted.

### 3.20 Tracked-debt comment cleanup
- **Files:** [src/config/GpuDetector.ts](../../../../versions/src/config/GpuDetector.ts), [src/chat/ContextCompactor.ts](../../../../versions/src/chat/ContextCompactor.ts), [src/observability/GoldenTaskSuite.ts](../../../../versions/src/observability/GoldenTaskSuite.ts), [src/storage/UnifiedMemoryRetriever.ts](../../../../versions/src/storage/UnifiedMemoryRetriever.ts)

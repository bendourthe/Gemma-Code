...
**Date**: 2026-04-25
**Plan**: [docs/archive/versions/v0/v0.5.0/plans/implementation-plan.md](../../plans/implementation-plan.md) (Phase 10)
...
| # | Sub-task | Output |
|---|----------|--------|
| 10.1 | husky pre-commit + commit-msg | [.husky/pre-commit](../../../../versions/.husky/pre-commit), [.husky/commit-msg](../../../../versions/.husky/commit-msg), [scripts/hooks/check-commit-msg.mjs](../../../../versions/scripts/hooks/check-commit-msg.mjs), [package.json](../../../../versions/package.json) lint-staged config |
| 10.2 | dependency-cruiser baseline | [configs/dependency-cruiser.cjs](../../../../versions/configs/dependency-cruiser.cjs) with 4 hard rules + 3 soft rules + grandfathered baseline exceptions |
| 10.3 | Dependabot weekly config | [.github/dependabot.yml](../../../../versions/.github/dependabot.yml) (npm + github-actions + pip ecosystems; grouped weekly Monday 06:00 UTC) |
| 10.4 | ESLint ban-ts-comment | [eslint.config.mjs](../../../../versions/eslint.config.mjs) `allow-with-description` + `minimumDescriptionLength: 20` |
| 10.5 | SHA-pin GitHub Actions | All `uses:` across [ci.yml](../../../../versions/.github/workflows/ci.yml), [nightly.yml](../../../../versions/.github/workflows/nightly.yml), [golden-tasks.yml](../../../../versions/.github/workflows/golden-tasks.yml), [release.yml](../../../../versions/.github/workflows/release.yml), [installer-smoke.yml](../../../../versions/.github/workflows/installer-smoke.yml) pinned to 40-char SHAs |
| 10.6 | Workflow concurrency cancellation | `cancel-in-progress: true` added to [nightly.yml](../../../../versions/.github/workflows/nightly.yml) (other long workflows already had it; release.yml deliberately excluded) |
| 10.7 | Node-version CI matrix | `strategy.matrix.node: [18.x, 20.x, 22.x]` on lint-ts/test-ts/build-ts in [ci.yml](../../../../versions/.github/workflows/ci.yml); `engines.node: ">=18.0.0"` in [package.json](../../../../versions/package.json) |
...
`dependency-cruiser@^16` config at [configs/dependency-cruiser.cjs](../../../../versions/configs/dependency-cruiser.cjs).
...
[.github/dependabot.yml](../../../../versions/.github/dependabot.yml) configures three ecosystems on weekly Monday 06:00 UTC:
...
`@typescript-eslint/ban-ts-comment` configured in [eslint.config.mjs](../../../../versions/eslint.config.mjs):
...
The current codebase has zero TS suppressions in `src/`, so the rule is forward-only. A meta-test at [tests/unit/lint-discipline.test.ts](../../../../versions/tests/unit/lint-discipline.test.ts) walks every `.ts` file under `src/` to assert this property.
...
Meta-test [tests/unit/workflow-discipline.test.ts](../../../../versions/tests/unit/workflow-discipline.test.ts) walks every workflow file and asserts every `uses:` reference satisfies `^[0-9a-f]{40}$` after the `@`.
...
[ci.yml](../../../../versions/.github/workflows/ci.yml) lint-ts, test-ts, and build-ts jobs gain:
...
`engines.node: ">=18.0.0"` set in [package.json](../../../../versions/package.json). No syntax in the codebase requires Node 20+ features; verified by green Node 18 runs in CI.
...
Documented in the [DEVLOG entry](../../../../versions/v0/DEVLOG.md). Summary:
...
Twelve test failures in [tests/unit/chat/CompactionStrategy.test.ts](../../../../versions/tests/unit/chat/CompactionStrategy.test.ts), [tests/unit/chat/ContextCompactor.test.ts](../../../../versions/tests/unit/chat/ContextCompactor.test.ts), and [tests/unit/errors/error-handling.test.ts](../../../../versions/tests/unit/errors/error-handling.test.ts) predate Phase 10 (verified: `git diff c4944d5 -- <file>` is empty). Compaction failures look like tiktoken-vs-heuristic divergence after Phase 5; the error-handling failure is unrelated to Phase 10's scope. Phase 10 changes do not touch [src/chat/CompactionStrategy.ts](../../../../versions/src/chat/CompactionStrategy.ts), [src/chat/ContextCompactor.ts](../../../../versions/src/chat/ContextCompactor.ts), or [src/errors/](../../../../versions/src/errors). Per AGENTS.md ("Every changed line must trace directly to the user's request"), Phase 10 does not silently fix them. Tracking for a future phase (likely Phase 12 release gate).
...
- `docs/archive/versions/v0/v0.5.0/development/history/2026-04_phase-10-hygiene-and-ci-hardening.md` (this file)
...
- `.gitignore` -- ignore generated `docs/archive/versions/v0/v0.5.0/dep-graph.svg`

...
| Versioned doc tree | `docs/archive/versions/v0/v0.1.0/` through `docs/archive/versions/v0/v0.4.0/` (architecture, implementation plans, dev history, security audits, performance benchmarks, golden tasks) | None |
...
| Aspect | Gemma Code | free-claude-code |
|--------|------------|------------------|
| Unit/integration/e2e split | 78/6/6 (86.7/6.7/6.7%) per `docs/archive/versions/v0/v0.4.0/test-pyramid.md` | 68 deterministic + 35 smoke files |
...
| Adopt smoke-test classification rubric (skip on `missing_env`/`upstream_unavailable`, fail on `product_failure`/`harness_bug`) | `PLAN.md` smoke coverage policy + `smoke/conftest.py` | `tests/integration/` test docstrings + `docs/archive/versions/v0/v0.4.0/test-pyramid.md` addendum | Medium | None | Low — formalizes existing skip behavior |

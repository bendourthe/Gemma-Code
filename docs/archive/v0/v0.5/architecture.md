...
This document captures the v0.5.0 architecture: the canonical directives, the harness layer, the persistent-cache stack, the memory-consolidation rule, and the new tool-surface parameters that landed across phases 1-12. It complements [ARCHITECTURE.md](../../../ARCHITECTURE.md) (which stays version-neutral) and supersedes [docs/archive/versions/v0/v0.3.0/architecture.md](../v0.3/architecture.md) for the v0.5.0 release.
...
- The five comparison-driven adoption plans live under [docs/archive/versions/v0/v0.5.0/comparison/](comparison) and the executable plans under [docs/archive/versions/v0/v0.5.0/plans/](plans). The canonical execution path is [docs/archive/versions/v0/v0.5.0/plans/implementation-plan.md](plans/implementation-plan.md); sub-plans are reference material.
...
| 2 -- dangerous | `browser_aria_snapshot`, `browser_click`, `browser_close`, `browser_navigate`, `browser_type`, `fetch_page`, `run_terminal`, `web_search` | Blocking confirmation; `editMode: plan` shows a diff |
...
- semantic-release on push to `main` ([.releaserc.json](../../versions/.releaserc.json), [.github/workflows/semantic-release.yml](../../../.github/workflows/semantic-release.yml)); plugin chain `changelog -> git -> github`. No `@semantic-release/npm` -- Gemma ships as a VSIX
...
See [docs/adr/](../../adr) for the canonical text. The post-cycle architecture document for v0.6.0 lives at [docs/archive/versions/v0/v0.6.0/architecture.md](../v0.6/architecture.md).

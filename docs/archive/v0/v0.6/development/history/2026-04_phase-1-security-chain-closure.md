...
- **Prior session reference**: [docs/archive/versions/v0/v0.5.0/development/history/2026-04_phase-12-advanced-fallbacks-and-release-gate.md](../../../v0.5/development/history/2026-04_phase-12-advanced-fallbacks-and-release-gate.md) (v0.5.0 final phase)
- **Plan reference**: [docs/archive/versions/v0/v0.6.0/plans/v0.6.0-cycle.md](../../plans/v0.6.0-cycle.md), Phase 1 (sub-tasks 1.1, 1.2, 1.3, 1.4)
...
**Plan specification**: Replace the body of `resolveWorkspacePath` in [src/tools/handlers/filesystem.ts](../../../../versions/src/tools/handlers/filesystem.ts) with a thin delegation to `resolveInsideWorkspace` from [src/tools/handlers/pathGuard.ts](../../../../versions/src/tools/handlers/pathGuard.ts). Verify the seven filesystem tool handlers still receive a string. Add `tests/unit/tools/handlers/filesystem-symlink.test.ts` proving every handler refuses a workspace-internal symlink that resolves outside the workspace root. Acceptance: pen-test F-001 closed; symlink leg of Attack Path A broken.

**What happened**: 

1. Replaced the body of `resolveWorkspacePath` in [src/tools/handlers/filesystem.ts](../../../../versions/src/tools/handlers/filesystem.ts) (lines 43-51 originally) with `return resolveInsideWorkspace(relativePath);`. Added a delegating `workspaceRoot()` wrapper that calls the imported `guardedWorkspaceRoot` to keep the local function name available.
...
3. Updated the `Path traversal` substring in [tests/unit/tools/handlers/filesystem.test.ts](../../../../versions/tests/unit/tools/handlers/filesystem.test.ts) line 95 to `resolves outside the workspace` to match the new error message coming from `pathGuard.resolveInsideWorkspace`.
4. Wrote the new test file [tests/unit/tools/handlers/filesystem-symlink.test.ts](../../../../versions/tests/unit/tools/handlers/filesystem-symlink.test.ts): builds a real workspace + outside dir on disk, creates a symlink/junction `inner -> outsideRoot`, mocks `vscode.workspace.workspaceFolders` per the existing pattern in [tests/integration/dry-run-end-to-end.test.ts](../../../../versions/tests/integration/dry-run-end-to-end.test.ts), wires `mockFs` to the real filesystem, and asserts each of the 7 filesystem tools refuses paths through the symlink. Added a runtime probe that detects whether the host can create symlinks (Windows requires admin or Developer Mode; falls back to junctions for directory targets).
...
**Plan specification**: In [src/guardrails/PermissionTiers.ts](../../../../versions/src/guardrails/PermissionTiers.ts), find the path that reads `permissionOverrides[toolName]`. Add a clamp: if the tool's baseline tier is 2 and the requested override is < 1, log a warning via `getLogger()` and use 1. Add `tests/integration/permission-overrides-clamp.test.ts` covering run_terminal, delete_file, and read_file. Update the `gemma-code.permissionOverrides` description in [package.json](../../../../versions/package.json) to mention the floor. Acceptance: pen-test F-003 closed; auto-approve leg of Attack Path A broken.
...
1. Added `ToolCallSource = "local-agent" | "sub-agent" | "mcp"` to [src/tools/types.ts](../../../../versions/src/tools/types.ts) and an optional `source?: ToolCallSource` field on `ToolCall`.
2. Updated [src/tools/ConfirmationGate.ts](../../../../versions/src/tools/ConfirmationGate.ts) `request(id, description, detail, source?)` to prefix the description: `"External MCP client wants to: ..."` for `mcp`, `"The verification sub-agent wants to: ..."` for `sub-agent`, no prefix for local-agent or undefined.
3. Updated [src/tools/ToolRegistry.ts](../../../../versions/src/tools/ToolRegistry.ts) `execute()` to thread `call.source` into the gate request as the 4th argument.
4. Added `AgentLoopOptions.toolCallSource` to [src/tools/AgentLoop.ts](../../../../versions/src/tools/AgentLoop.ts); the dispatch site now passes `call.source ?? this._toolCallSource` so a constructor-level default can stamp every call.
5. Updated [src/agents/SubAgentManager.ts](../../../../versions/src/agents/SubAgentManager.ts) to construct the inner `AgentLoop` with `{ toolCallSource: "sub-agent" }`.
6. Added `DEFAULT_MCP_EXPOSED_TOOLS = ["read_file", "list_directory", "grep_codebase"]` constant to [src/mcp/McpServer.ts](../../../../versions/src/mcp/McpServer.ts). Constructor now accepts an optional `exposedTools` allowlist (defaults to the constant). The `start()` loop filters every catalog entry against the allowlist before registering with the SDK, and stamps `source: "mcp"` on every dispatched call.
7. Added the `gemma-code.mcpExposedTools` setting to [package.json](../../../../versions/package.json). Updated [src/config/settings.ts](../../../../versions/src/config/settings.ts) to load it. Updated [src/panels/GemmaCodePanel.ts](../../../../versions/src/panels/GemmaCodePanel.ts) to forward `settings.mcpExposedTools` to the `McpServer` constructor.
8. Added a `peer attribution` describe block to [tests/unit/tools/ConfirmationGate.test.ts](../../../../versions/tests/unit/tools/ConfirmationGate.test.ts) with 3 cases (no prefix for local-agent/undefined, MCP prefix, sub-agent prefix).
9. Added two new cases to [tests/unit/mcp/McpServer.test.ts](../../../../versions/tests/unit/mcp/McpServer.test.ts): allowlist filtering, and `source: "mcp"` propagation through the SDK callback.
10. Updated [docs/archive/versions/v0/v0.5.0/architecture.md](../../../v0.5/architecture.md) Section 2 with the new harness-layer principle (item 3, ahead of the existing Specialist prompts item).

**Key files changed**: `src/tools/types.ts`, `src/tools/ConfirmationGate.ts`, `src/tools/ToolRegistry.ts`, `src/tools/AgentLoop.ts`, `src/agents/SubAgentManager.ts`, `src/mcp/McpServer.ts`, `src/panels/GemmaCodePanel.ts`, `src/config/settings.ts`, `package.json`, `tests/unit/tools/ConfirmationGate.test.ts`, `tests/unit/mcp/McpServer.test.ts`, `docs/archive/versions/v0/v0.5.0/architecture.md`
...
- **`/update-gitignore`** -> [docs/git/gitignore-audit-2026-04-26.md](../../../../versions/v0/git/gitignore-audit-2026-04-26.md) -- zero findings (5th consecutive clean audit).
- **`/update-devlog`** -> prepended a 2026-04-26 entry to [docs/DEVLOG.md](../../../../versions/v0/DEVLOG.md) covering goals, the four approaches taken, the realpath-ancestor-walk fix, and the segfault discovery.
- **`/update-documentation`** -> updated [SECURITY.md](../../../../versions/SECURITY.md) (Supported Versions bump 0.4 -> 0.5/0.6; path-guard line + MCP allowlist + permissionOverrides floor paragraphs added; 2 new rows in Security-Related Configuration table); [README.md](../../../../versions/README.md) (2 new rows in settings table for `mcpExposedTools` + `permissionOverrides`); [docs/todos.md](../../../../versions/v0/todos.md) (added v0.5.0 shipped section + v0.6.0 in-progress section with Phase 1 checked off).
...
- [x] 1.3 Update [docs/archive/versions/v0/v0.5.0/architecture.md](../../../v0.5/architecture.md) Section 2 with the new harness-layer principle

...
1. [tests/benchmarks/predictive-cache.bench.ts](../../../../versions/tests/benchmarks/predictive-cache.bench.ts) -- 3 bench cases on `PredictiveCache.observe()`, `predict(5)`, and `forecastARIMA101`. Deterministic 1000-observation Zipf-like trace generated inline from a fixed RNG seed.
2. [tests/unit/storage/PredictiveCache.budget.test.ts](../../../../versions/tests/unit/storage/PredictiveCache.budget.test.ts) -- 2 `it()` assertions for the 50 ms ARIMA budget. **Real gates** (run under `vitest run`), not just bench documentation.
3. [tests/benchmarks/eviction-strategies.bench.ts](../../../../versions/tests/benchmarks/eviction-strategies.bench.ts) -- 5 `bench()` cases (LRU, LFU, ARC, W-TinyLFU, Clock) replaying the fixture trace. Plus 6 smoke `it()` assertions documenting hit-rate ordering. Cache size 16 against 64-path / 2048-access Zipf trace.
4. [tests/integration/heuristic-fallback.test.ts](../../../../versions/tests/integration/heuristic-fallback.test.ts) -- 3 `it.todo` assertions for the F-007 threshold-elevation contract. Intentionally `it.todo` until Phase 5 sub-task 5.1 lands; marked in the doc comment so the next maintainer knows when to flip them.
5. [tests/fixtures/access-trace.json](../../../../versions/tests/fixtures/access-trace.json) -- 2048-entry Zipfian access trace (skew 1.1, 64 paths, deterministic RNG seed `0x12345678`) consumed by `eviction-strategies.bench.ts`. Generated programmatically; replace with a golden-task-derived trace once cache instrumentation lands.
...
|---|---|---|---|
| v0.4.0 | git worktree at tag `v0.4.0` (commit `ef6d8b3`); `npm ci`; `npm run bench` | 12 | [tests/benchmarks/baselines/v0.4.0.json](../../../../versions/tests/benchmarks/baselines/v0.4.0.json) (replaces empty seed-time placeholder) |
| v0.5.0 | git worktree at tag `v0.5.4` (commit `2ca1625`); `npm ci`; `npm run bench` | 18 | [tests/benchmarks/baselines/v0.5.0.json](../../../../versions/tests/benchmarks/baselines/v0.5.0.json) (new file) |
| v0.6.0 | main branch post-Phase-1; `npm run bench` (includes the two new bench files from sub-task 2.5) | 28 | [tests/benchmarks/baselines/v0.6.0.json](../../../../versions/tests/benchmarks/baselines/v0.6.0.json) (new file) |
...
The plan asks for `tests/golden/baselines/v0.4.0.json`, `v0.5.0.json`, and `v0.6.0.json` to be generated by `python tests/golden/framework/run_all.py --mode live`. The runner's `_run_live()` (at [tests/golden/framework/task_runner.py](../../../../versions/tests/golden/framework/task_runner.py) line 79) does:

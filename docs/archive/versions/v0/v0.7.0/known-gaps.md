...
**Sibling reviews**: [docs/archive/versions/v0/v0.6.0/review/known-gaps.md](../v0.6.0/review/known-gaps.md) (the v0.5.0 carryover catalog that drove v0.6.0); [docs/archive/versions/v0/v0.6.0/review/codebase-review.md](../v0.6.0/review/codebase-review.md); [docs/archive/versions/v0/v0.6.0/review/penetration-test.md](../v0.6.0/review/penetration-test.md).
...
### 5.3 `docs/archive/versions/v0/v0.7.0/comparison-multi-source.md` and `docs/archive/versions/v0/v0.7.0/plans/` were untracked at Phase 8 start

**Severity**: P3
**Files**: `docs/archive/versions/v0/v0.7.0/comparison-multi-source.md`, `docs/archive/versions/v0/v0.7.0/plans/adoption-multi-source.md`, `docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md`
...
4. Record any *new* findings from a fresh codebase review / security audit pass at `docs/archive/versions/v0/v0.7.0/review/`.

A draft v0.7.0 cycle plan exists at [docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md](./plans/v0.7.0-cycle.md) (untracked at v0.6.0 ship; see Section 5.3).
...
|---|---|---|---|---|---|---|
| 10.O.1 | Phase 4 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 4 stability gate | NI | P1 | Panel host does not yet swap the input row for the queued-message field during streaming. The render primitive ships; the swap is intentionally deferred. | Wire `renderQueuedMessageField` toggle in [src/panels/ChatWebviewHost.ts](../../../../src/panels/ChatWebviewHost.ts) from the streaming start / end events. |
| 10.O.2 | Phase 4 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 4 stability gate | NI | P1 | `ChatMessageRouter` does not yet route `permissionPromptResponse` to `ConfirmationGate.resolvePrompt`. | Add the `permissionPromptResponse` case to [src/panels/ChatMessageRouter.ts](../../../../src/panels/ChatMessageRouter.ts) and resolve via the gate. |
| 10.O.3 | Phase 4 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 4 stability gate | NI | P1 | `ToolRegistryBuilder.todos` is not yet wired into panel bootstrap. | Toggle `todos: true` for the active session inside [src/panels/ChatPanelBootstrap.ts](../../../../src/panels/ChatPanelBootstrap.ts) registry construction. |
| 10.O.4 | Phase 5 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 5 sub-task 5.3 | DF | P3 | Plan called for ADR-0007 but `0007-permission-tier-floor.md` was already shipped. ADR landed as ADR-0014. | None. The deviation is documented inline in [docs/adr/0014-memory-file-architecture.md](../../../adr/0014-memory-file-architecture.md) "Numbering note" and mirrors the same pattern Phase 4 used for ADR-0013 (plan said 0008). Phase 8 must update the plan's ADR cross-references. |
| 10.O.5 | Phase 5 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 5 stability gate item (e) | DF | P2 | The MemoryPanel's "Promote to Memory.md" action picks the section heading via a static `sectionForType` mapping (`decision -> Decisions`, `preference -> Preferences`, `error_resolution -> Corrections`, `file_pattern -> Patterns`, fallback `Preferences`). The plan did not specify a target section; this is a deliberate UX choice. | If user feedback in v0.7.0 testing surfaces a different mapping, revisit before v0.7.0 ship. Otherwise document in CHANGELOG. |
| 10.O.6 | Phase 5 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 5 stability gate | DF | P3 | "Finalize per-model context limits" was already accomplished in Phase 3 sub-task 3.7. Phase 5 has no additional code to add. | Document in Phase 5 devlog and architecture.md cross-link. |
| 10.O.7 | Phase 6 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 6 sub-task 6.1 | DF | P2 | The Cursor adapter emits `.cursor/rules/<slug>.md` with a placeholder `rule: SKILL` frontmatter rather than Cursor's native `.cursor/rules/<slug>.mdc` (with `description` / `globs` / `alwaysApply`). The plan acknowledged the schema gap and accepted a best-effort transform; the bundled README documents the limitation. A fully-native Cursor conversion is deferred. | Survey actual Cursor rule consumption in dogfood, design a real mapping (likely `description` <- SKILL `description`, `alwaysApply` defaulted, body inlined), and migrate before v0.8.0 or after first user report. |
| 10.O.8 | Phase 6 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 6 sub-task 6.2 | NI | P3 | The plan listed `no-bare-promise-rejection` (regex match `\.catch(\s*\)`) as an optional 5th rule. Not shipped; the 4 mandatory rules are in. | Add `lib/checks/no-bare-promise-rejection.mjs` mirroring the existing rule contract if a real-world incident appears, or leave for v0.8.0. |
| 10.O.9 | Phase 6 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 6 stability gate | WN | P3 | `npm run deps:check` reports 4 pre-existing dependency-cruiser violations: 3 `no-storage-from-panels` (MemoryPanel imports MemoryStore / MemoryShared.types / MemoryFiles) and 1 `no-panels-from-tools` (ConfirmationGate imports webview/render/permissionPrompt). The count is unchanged before vs. after Phase 6 (verified via `git stash` baseline). Carryovers from Phases 4 / 5. | Address in Phase 8 ratchet pass: either relax the rule with an inline `dependency-cruiser-disable-next-line` for the MemoryPanel case (it owns the store by design) and refactor ConfirmationGate to receive a prompt-renderer callback rather than import the panel module, or accept and update the rule. |
| 10.O.10 | Phase 6 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 6 sub-task 6.2 | WN | P3 | The legacy `scripts/check-bench-regressions.mjs` (~6 `console.log` calls) and `scripts/hooks/check-prompt-policy.mjs` + `scripts/hooks/lib/secret-paths.mjs` are flagged by `gemma-check` because they contain `console.log` and inline pattern definitions matching the new rules. The Phase 6 acceptance gate runs on `src/` only (which is clean); the script files are intentionally out of scope. | Either convert the legacy scripts' `console.log` to `process.stdout.write` (preferred, matches project style) or add `gemma-check-allow` markers on the pattern-definition lines. Not blocking; opportunistic cleanup. |
| 10.O.11 | Phase 7 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.1 | MT | P2 | The HNSW recall-delta and HNSW-load-failure integration tests are gated on `hnswlib-node` being loadable at test time. On the local Windows dev workstation and in CI the optionalDependency does not install, so `it.runIf(HNSW_AVAILABLE)` skips the loaded-path tests. Coverage of the linear-scan fallback contract is exercised; the loaded HNSW path is only verified by the always-on graceful-fallback test. | Run the test suite on a machine where `hnswlib-node` installs cleanly (Linux x64 or macOS) and confirm the 5 skipped MemoryHnswIndex + 2 skipped memory-hnsw integration tests pass. Capture the result in v0.8.0 Phase 0 close-out. |
| 10.O.12 | Phase 7 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.2 | MT | P2 | The background-worker triggers in AgentLoop are exercised at the unit level with a mock subAgentManager. The end-to-end path -- AgentLoop -> SubAgentManager.run -> BackgroundWorkers.runAuditWorker -> spawn `node bin/gemma-check.mjs --json` -> chat message render -- is NOT covered by an integration test. The deterministic-runner injection makes a real E2E test feasible but it was not added in Phase 7. | Add `tests/integration/background-workers-end-to-end.test.ts` covering one audit-worker run with `gemma-check` against a fixture file and one testgaps-worker run against a tiny vitest sample. Defer to v0.8.0 Phase 7 if scope is tight. |
| 10.O.13 | Phase 7 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.1 | DF | P3 | `hnswlib-node` is listed as an `optionalDependency` but `npm install` was not re-run in Phase 7 (the dependency check is the operator's responsibility under v0.7.0 known-gaps Section 1.1). The package-lock has not been regenerated. | Operator: run `npm install` on the next cycle close-out; verify `hnswlib-node` either installs or is gracefully skipped on the target platform; commit the resulting `package-lock.json` change. |
| 10.O.14 | Phase 8 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md sub-task 8.1 | DF | P1 | `tests/golden/baselines/v0.7.0.json` ships as a `status: deferred-to-operator` placeholder. Live-Ollama capture requires `ollama serve` with `gemma4:e4b` pulled on a quiescent dev workstation; the Phase 8 author does not have access to the model layer. The plan also assumed a TS-native golden runner that was never built during the cycle (the existing Python framework in `tests/golden/framework/` is the only runner). Same precedent as v0.6.0 known-gaps Section 1.1 (whose v0.6.0 baseline is also still pending operator capture). | Operator: run `python tests/golden/framework/run_all.py --model gemma4:e4b --output tests/golden/baselines/v0.7.0.json` on a quiescent workstation; then run the regression check against `tests/golden/baselines/v0.6.0.json` (also a pending capture; both should land together). Build the TS-native golden runner in v0.8.0 if cycle scope permits. |
| 10.O.15 | Phase 8 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md sub-task 8.1 | BG | P2 | The captured `tests/benchmarks/baselines/v0.7.0.json` shows a uniform 30-80% hz drop across every retained benchmark (cache, eviction, hooks, rendering, skill-loading, file reading) versus the v0.6.0 baseline. The signature (uniform across unrelated subsystems) is inconsistent with any single v0.7.0 code change and is most consistent with CPU pressure / thermal throttling / background-process noise on a non-quiescent capture host. The plan explicitly requires a "quiescent dev workstation" for the bench capture. | Operator: re-run `npm run bench -- --outputJson=tests/benchmarks/baselines/v0.7.0.json` on a quiescent workstation, then re-run `node scripts/check-bench-regressions.mjs --baseline tests/benchmarks/baselines/v0.6.0.json --current <newJson>` and either accept the new numbers as the canonical v0.7.0 baseline or document any genuine code-caused regression. |
| 10.O.16 | Phase 8 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 8 stability gate (implicit) | QG | P2 | `npm run deps:check` reports the same 4 pre-existing violations from Phase 6 / item 10.O.9 (3x `no-storage-from-panels` for MemoryPanel imports, 1x `no-panels-from-tools` for ConfirmationGate -> permissionPrompt). The Phase 8 release-gate review accepted these as pre-existing v0.7.0 internal carryovers rather than v0.6.0 -> v0.7.0 regressions and did not block on them. Already tracked as 10.O.9 transferred to v0.8.0 plan (Phase 7 appendix sub-task 7.B). | Duplicate of 10.O.9; v0.8.0 Phase 7 appendix 7.B is the canonical close-out. |
| 10.O.17 | Phase 8 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md sub-task 8.1 | NI | P3 | The plan referenced a TS-native golden runner as the v0.7.0-cycle deliverable ("if not yet built, this is the cycle to build it"). It was never built; the Python framework in `tests/golden/framework/` is still the only runner. No earlier phase formally absorbed this scope. | Defer to v0.8.0. Either explicitly accept the Python runner as canonical (and rewrite the plan reference) or build the TS-native runner before the v0.8.0 release gate. |
| 10.O.18 | Phase 8 hotfix | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.1 | BG | P2 | After the Phase 8 stryker pin re-alignment (`vitest-runner@^8.7.0` / `core@^8.7.0`) `npm install` finally pulled in the `hnswlib-node` optionalDependency on the local Windows workstation. With HNSW_AVAILABLE now true, the `tests/unit/storage/MemoryHnswIndex.test.ts > persists and reloads the index from disk` test surfaced a real bug: after `idx1.persist()` writes the file to disk and a fresh `MemoryHnswIndex.tryCreate` loads it via `readIndexSync`, `getCurrentCount()` returns 0 instead of 2 and `search()` returns []. The test was gated behind `HNSW_RUN_PERSIST=1` so neither local nor CI runs surface a regression while the root cause is investigated. The five other `runIf(HNSW_AVAILABLE)` tests in the same file still run. Affects only the loaded HNSW persistence path; the always-on linear-scan fallback is exercised by separate tests and remains the production-safe path. | Investigate whether the bug is Windows-specific (path separators, file locking) or a hnswlib-node v3 API change requiring `setEf` / `resizeIndex` after read. Likely fix is in [src/storage/MemoryHnswIndex.ts](../../../../src/storage/MemoryHnswIndex.ts) `tryCreate` between the `readIndexSync` call and the constructor return. Re-enable the test without the env-gate once the root cause is fixed. |
| 10.O.19 | v0.7.1 hotfix (post-Phase-8) | docs/archive/versions/v0/v0.7.0/known-gaps.md item 2.1 resolution | BG | P2 | The Phase 0 `marked` v4 -> v12 bump (closed as 2.1) traded `~50%` of `MarkdownRenderer` throughput for the modernization narrative. Nightly bench gate against v0.4.0 baseline isolated the signal once the v0.7.1 stryker hotfix unblocked CI: `render ~100-token message` 1284.85 -> 504.17 hz (-60.8%), `render ~500-token message` 463.72 -> 238.39 hz (-48.6%), `render ~2000-token message` 128.47 -> 70.17 hz (-45.4%). All non-renderer benches stayed within band, confirming the Phase 8 `uniform 30-80%` capture noise (10.O.15) was non-quiescent-host artifact; the underlying renderer regression is real and reproduces on a quiet Linux runner. User-facing impact is negligible (a 2000-token chat message renders in ~14 ms on the CI runner, still under a single 60 Hz frame), so the regression is accepted and suppressed via `--exclude '^render ~.*-token message$'` in the nightly bench gate while the root cause is investigated. The Phase 0 claim "All 8 renderer tests green; sanitization chain intact" was correct as a functional acceptance criterion; it did not include a perf gate. | Two parallel actions for v0.8.0: (1) profile `src/utils/MarkdownRenderer.ts` to identify whether the cost lives in `marked.parse` setup-per-call, the `walkTokens` hook, or our three custom renderers. Likely fix candidates: cache the configured `marked` instance, pre-warm the lexer, or replace the `{ async: false }` wrapper with the direct `marked(text)` shorthand. (2) If profiling shows the regression is intrinsic to v12, evaluate downgrading to a specific v12.x.y release known to be faster, or revert to v4 and document the sanitization-chain trade-off (DOMPurify already provides that layer per Phase 0 resolution note). Re-include the render benches in the gate once parity is reached. |

### 10.2 Resolved

| ID | Source phase | Plan reference | Category | Resolved in | Notes |
|---|---|---|---|---|---|
| 10.O.1 | Phase 4 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 4 stability gate | NI | resolved in v0.8.0 Phase 0.3 (2026-05-15) | Closed in [docs/archive/versions/v0/v0.8.0/known-gaps.md](../v0.8.0/known-gaps.md) Section 10.2. `ChatWebviewHost.postMessage` now emits `renderQueuedMessageField` toggles on every `status` transition; regression tests in `tests/unit/panels/ChatWebviewHost.test.ts`. |
| 10.O.2 | Phase 4 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 4 stability gate | NI | resolved in v0.8.0 Phase 0.4 (2026-05-15) | Closed. `ChatMessageRouter` now routes `permissionPromptResponse` to `ConfirmationGate.resolvePrompt`; integration tests in `tests/integration/panels/permissionPrompt.test.ts`. |
| 10.O.3 | Phase 4 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 4 stability gate | NI | resolved in v0.8.0 Phase 0.5 (2026-05-15) | Closed. `ChatPanelBootstrap` now constructs `TodoState` and wires `todos` into `buildToolRegistry`, registering `update_todos`; regression tests in `tests/unit/panels/ChatPanelBootstrap.test.ts`. |
| 10.O.4 | Phase 5 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 5 sub-task 5.3 | DF | Resolved in v0.8.0 Phase 7.1 | Closed. The v0.7.0 cycle plan's provisional ADR numbers (ADR-0006 / ADR-0007 / ADR-0008) were rewritten to the actual shipped numbers (ADR-0012 compress tool, ADR-0013 webview render protocol, ADR-0014 memory file architecture) across Sections 4.8, 5.3, 3.8, and the Phase-table summary rows; each updated line cites the v0.8.0 Phase 7.1 fix inline. |
| 10.O.5 | Phase 5 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 5 stability gate item (e) | DF | Resolved in v0.8.0 Phase 5.10 | Mapping documented at `docs/archive/versions/v0/v0.8.0/memory-promotion-mapping.md`; overridable via the `gemma-code.memory.promotionMapping` setting; `sectionForType` accepts an override map. |
| 10.O.6 | Phase 5 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 5 stability gate | DF | Resolved in v0.8.0 Phase 5.11 | Architecture cross-link added: `docs/archive/versions/v0/v0.7.0/architecture.md` Phase 5 section now points at the actual Phase 3 sub-task 3.7 that shipped per-model context limits. |
| 10.O.7 | Phase 6 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 6 sub-task 6.1 | DF | transferred to v0.8.0 plan (Phase 6 appendix sub-task 6.A) | Carried to v0.8.0; close when Cursor adapter emits native `.mdc` rules. |
| 10.O.8 | Phase 6 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 6 sub-task 6.2 | NI | Resolved in v0.8.0 Phase 7.A | Closed. `lib/checks/no-bare-promise-rejection.mjs` added and registered in `lib/checks/index.mjs`; pattern `\.catch\s*\(\s*\)` flagged as a warning outside test files; 3 unit tests + a registry-check test at `tests/unit/lib/no-bare-promise-rejection.test.ts`. |
| 10.O.9 | Phase 6 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 6 stability gate | WN | Resolved in v0.8.0 Phase 7.B | Closed. (a) `MemoryPanel` added to the `no-storage-from-panels` whitelist in `configs/dependency-cruiser.cjs` with documented rationale + inline `dependency-cruiser-disable-next-line` markers on the three storage imports. (b) `ConfirmationGate` refactored to accept a `PermissionOptionsBuilder` callback via constructor; the panel-side `defaultPermissionOptions` import moved to the call site in `ChatPanelBootstrap`. (c) Regression test at `tests/integration/dep-cruiser-clean.test.ts` asserts `depcruise` exits 0. |
| 10.O.10 | Phase 6 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 6 sub-task 6.2 | WN | Resolved in v0.8.0 Phase 7.C | Closed. All `console.log` calls in `scripts/check-bench-regressions.mjs` converted to `process.stdout.write` with explicit newlines, preserving formatting and error semantics. `scripts/hooks/check-prompt-policy.mjs` and `scripts/hooks/lib/secret-paths.mjs` audited and confirmed clean (no remaining `console.log`). |
| 10.O.11 | Phase 7 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.1 | MT | transferred to v0.8.0 plan (Phase 0.10) | Carried to v0.8.0; close when the operator runs the suite on a platform where `hnswlib-node` installs cleanly and the gated HNSW tests run green. |
| 10.O.12 | Phase 7 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.2 | MT | resolved in v0.8.0 Phase 0.11 (2026-05-15) | Closed. New `tests/integration/background-workers-end-to-end.test.ts` exercises `runAuditWorker` end-to-end against a fixture file with a seeded `no-secret-patterns` finding via the real `bin/gemma-check.mjs` spawn. |
| 10.O.13 | Phase 7 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.1 | DF | transferred to v0.8.0 plan (Phase 0.10) | Carried to v0.8.0; close when `npm install` re-runs and `package-lock.json` is regenerated with `hnswlib-node` resolved (or gracefully skipped). |
| 10.O.14 | Phase 8 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md sub-task 8.1 | DF | transferred to v0.8.0 plan (Phase 0.12) | Carried to v0.8.0; close when the operator captures `tests/golden/baselines/v0.7.0.json` against live Ollama on a quiescent workstation. |
| 10.O.15 | Phase 8 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md sub-task 8.1 | BG | transferred to v0.8.0 plan (Phase 0.12) | Carried to v0.8.0; close when the bench baseline is re-captured on a quiescent workstation and the regression check vs. v0.6.0 either runs green or documents a real code regression. |
| 10.O.16 | Phase 8 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 8 stability gate (implicit) | QG | duplicate of 10.O.9 | Already transferred via 10.O.9 to v0.8.0 plan (Phase 7 appendix sub-task 7.B). |
| 10.O.17 | Phase 8 | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md sub-task 8.1 | NI | resolved in v0.8.0 Phase 0.13 (2026-05-15) | Closed. [ADR-0017](../../../adr/0017-golden-runner-disposition.md) canonises the Python runner. README + CONTRIBUTING golden-suite sections updated accordingly. |
| 10.O.18 | Phase 8 hotfix | docs/archive/versions/v0/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.1 | BG | resolved in v0.8.0 Phase 0.8 (2026-05-15) | Closed. Root cause was passing `maxElements` (number) where hnswlib-node v3 `readIndexSync` expects `allowReplaceDeleted: boolean`. Truthy-coercion silently enabled deletion-replacement mode and zeroed `getCurrentCount()`. Fix in [src/storage/MemoryHnswIndex.ts](../../../../src/storage/MemoryHnswIndex.ts); `HNSW_RUN_PERSIST` env-gate removed from `tests/unit/storage/MemoryHnswIndex.test.ts`. |
| 10.O.19 | v0.7.1 hotfix | docs/archive/versions/v0/v0.7.0/known-gaps.md item 2.1 resolution | BG | resolved in v0.8.0 Phase 0.9 (2026-05-15) | Closed. `MarkdownRenderer` now caches a single `new Marked()` instance instead of using `marked.parse()` shorthand. `--exclude '^render ~.*-token message$'` rule removed from `.github/workflows/nightly.yml`. Post-fix bench numbers tracked in [docs/archive/versions/v0/v0.8.0/performance-baselines.md](../v0.8.0/performance-baselines.md) (operator-capture pending). |

All nineteen open items have been transferred to the v0.8.0 plan. The items remain unresolved (the work has not been done); they are tracked in a new surface so the v0.7.0 in-cycle log reaches its terminal state. When each v0.8.0 sub-task lands, the corresponding entry in `docs/archive/versions/v0/v0.8.0/known-gaps.md` Section 10 is resolved as the canonical source.
...
**Status**: all items transferred (13 from Phases 4-7 close + 5 net new from Phase 8 / Phase 8 hotfix with one a 10.O.9 duplicate). The v0.7.0 in-cycle log is closed; v0.8.0's `docs/archive/versions/v0/v0.8.0/known-gaps.md` is the canonical tracking surface going forward.

...
**Sibling docs**: [docs/archive/versions/v0/v0.8.0/known-gaps.md](../v0.8.0/known-gaps.md) Section 10.1 (the gap log this checklist drains); [docs/archive/versions/v0/v0.9.0/plans/v0.9.0-cycle.md](plans/v0.9.0-cycle.md) sub-task 1.3 (this file's spec); [docs/archive/versions/v0/v0.9.0/known-gaps.md](known-gaps.md) (authored at Phase 8.4 close; cross-links back here for the operator-only items)
...
6. Document deltas in `docs/archive/versions/v0/v0.9.0/performance-baselines.md` (create if missing). At minimum: token throughput per route, p50/p95 latency for retrieval, rendering benches.
...
3. Document the run host (kernel + arch + glibc) and result in a one-line note under Section 7 of `docs/archive/versions/v0/v0.9.0/performance-baselines.md` for traceability.
...
2. Update `docs/archive/versions/v0/v0.9.0/review/mutation-report.md` with the new overall + covered scores; the v0.7.0 baseline (50.64% overall, 58.92% covered) remains the floor.
...
1. Read `docs/archive/versions/v0/v0.6.0/review/penetration-test.md` Section 2 for the three Attack Paths verbatim:
...
2. For each Attack Path:
   ...
   - Record outcome in `docs/archive/versions/v0/v0.9.0/review/penetration-test.md` under one heading per Attack Path.
...
**Acceptance**: `docs/archive/versions/v0/v0.9.0/review/penetration-test.md` exists with one section per Attack Path; no unresolved P0/P1 finding; flip status above to `done`.
...
2. `git tag -a v0.8.0-cycle <sha> -m "v0.8.0 cycle close (see docs/archive/versions/v0/v0.8.0/known-gaps.md transferred-to-v0.9.0)"`
...
5. `gh release create v0.8.0-cycle gemma-code-*.vsix --title "v0.8.0 cycle close" --notes-file docs/archive/versions/v0/v0.8.0/CHANGELOG-CYCLE.md` (or `--notes` inline if the CHANGELOG-CYCLE file from 10.O.EE was not authored).
...
After every section flips to `done`, append a one-line note to [docs/archive/versions/v0/v0.8.0/known-gaps.md](../v0.8.0/known-gaps.md) Section 10.2 (Resolved) for the matching `10.O.<X>` entry with `Resolved in: v0.9.0 operator-actions Section <N>`, and recompute the Section 10.3 Summary counts.

...
Read the plan's Phase 1 section in full, then opened [docs/archive/versions/v0/v0.8.0/known-gaps.md](../../../v0.8.0/known-gaps.md) Section 10.1 to refresh on the 5 in-cycle gaps Phase 1 is designed to close (10.O.D, E, G, N, R) and the 8 operator-only carryovers (10.O.A / B / C / X / AA / BB / CC / DD). Confirmed vitest current version is `^1.0.0` per `package.json` and looked up the latest 2.x stable on npm: `vitest@2.1.9`.

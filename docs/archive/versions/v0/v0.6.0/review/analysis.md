...
**Confidence note**: this section is inferred from `dependency-cruiser.cjs` and `npm run deps:graph` output. The SVG dependency graph at [docs/archive/versions/v0/v0.5.0/dep-graph.svg](../../v0.5.0/dep-graph.svg) is the canonical artifact when it is generated.
...
- **Legacy v0.1.0 XML tool protocol** still documented under [docs/archive/versions/v0/v0.1.0/tool-protocol.md](../../v0.1.0/tool-protocol.md); product code uses the Gemma 4 native `<\|tool_call\|>` format exclusively since v0.2.0.
...
3. **[docs/archive/versions/v0/v0.5.0/architecture.md](../../v0.5.0/architecture.md)** -- the v0.5.0 design doc, including the cache stack, eviction strategies, embedding fallback, and the harness layer. Caveat: a few of its claims are inaccurate (see section 11).

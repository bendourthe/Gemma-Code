...
**Date**: 2026-04-25
**Plan**: [docs/v0.5.0/plans/implementation-plan.md](../../plans/implementation-plan.md) (Phase 1)
**Sub-plan reference**: [docs/v0.5.0/plans/ci-and-docs-hygiene.md](../../plans/ci-and-docs-hygiene.md) (Phase 1 sub-task 1.1; Phase 2 sub-tasks 2.1 + 2.2)
...
- Updated cross-references: [.vscodeignore](../../../../.vscodeignore) excludes `AGENTS.md` instead of `CLAUDE.md`; the `setup-project` skill at [src/skills/catalog/setup-project/SKILL.md](../../../../src/skills/catalog/setup-project/SKILL.md) now bootstraps an `AGENTS.md` for new projects; [CONTRIBUTING.md](../../../../CONTRIBUTING.md) names AGENTS.md as the canonical agent directive.
- Added [tests/unit/docs/AGENTS-md.test.ts](../../../../tests/unit/docs/AGENTS-md.test.ts) which asserts AGENTS.md exists and contains every required section, the cognitive-workflow vocabulary, and the no-co-author commit rule. The companion non-existence test asserts `CLAUDE.md` is gone.

### 1.2 - Smoke-test classification rubric

- Created [docs/v0.5.0/test-pyramid.md](../../test-pyramid.md) by carrying forward the v0.4.0 ratio documentation (86.7 / 6.7 / 6.7) and appending the new Smoke-Test Classification Rubric: `missing_env`, `upstream_unavailable`, `product_failure`, `harness_bug`. The rubric documents the helper functions canonical to the project and links to the meta-test.
- Extended [tests/helpers/factories.ts](../../../../tests/helpers/factories.ts) with `skipIfMissingEnv(...keys)` and `skipIfNoOllama()`. Both return booleans so they compose with `describe.skipIf(...)`.
- Reclassified [tests/integration/ollama-health.test.ts](../../../../tests/integration/ollama-health.test.ts) to use `skipIfNoOllama()` plus an inline class comment (`missing_env`). No other integration test had a bare env-var early return; the meta-test confirmed.
- Added [tests/unit/test-discipline.test.ts](../../../../tests/unit/test-discipline.test.ts) which walks `tests/integration/**/*.test.ts` and enforces: (a) no bare `if (!process.env.X) return` early returns; (b) any `it.skip(` / `describe.skip(` must have an adjacent `TODO(harness-bug)` or `TODO(missing_env)` comment. Bypass via `SKIP_TEST_DISCIPLINE_LINT=1` for emergency triage.
- Cross-referenced from [CONTRIBUTING.md](../../../../CONTRIBUTING.md) Testing section.
...
- Reference sweep: `git grep -i 'CLAUDE\.md'` against the active product surface (src/, tests/, root config files, AGENTS/CONTRIBUTING/ARCHITECTURE/README) returns zero matches. Remaining matches are confined to historical artifacts: `docs/DEVLOG.md` (past entries), `docs/v0.1.0`/`v0.2.0`/`v0.3.0` development history and reviews, and the v0.5.0 plan and comparison files which document the migration motivation.
...
- Added: [AGENTS.md](../../../../AGENTS.md), [docs/v0.5.0/test-pyramid.md](../../test-pyramid.md), [tests/unit/docs/AGENTS-md.test.ts](../../../../tests/unit/docs/AGENTS-md.test.ts), [tests/unit/test-discipline.test.ts](../../../../tests/unit/test-discipline.test.ts), this session-history file
- Modified: [.vscodeignore](../../../../.vscodeignore), [src/skills/catalog/setup-project/SKILL.md](../../../../src/skills/catalog/setup-project/SKILL.md), [CONTRIBUTING.md](../../../../CONTRIBUTING.md), [tests/helpers/factories.ts](../../../../tests/helpers/factories.ts), [tests/integration/ollama-health.test.ts](../../../../tests/integration/ollama-health.test.ts)
- Deleted: `CLAUDE.md`
...
- [x] `docs/v0.5.0/test-pyramid.md` includes the smoke-test classification rubric

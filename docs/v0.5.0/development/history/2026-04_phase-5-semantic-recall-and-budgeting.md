...
**Date**: 2026-04-25
**Plan**: [docs/v0.5.0/plans/implementation-plan.md](../../plans/implementation-plan.md) (Phase 5)
**Status**: Complete
...
**Files**:
- [src/storage/ToolOutputCache.ts](../../../../src/storage/ToolOutputCache.ts) (extended with embedding column, FTS5, `searchByEmbedding`, `searchByKeyword`)
- [src/storage/UnifiedMemoryRetriever.ts](../../../../src/storage/UnifiedMemoryRetriever.ts) (new `searchToolOutputs` method)
- [src/storage/MemorySubsystem.ts](../../../../src/storage/MemorySubsystem.ts) (threads ToolOutputCache + EmbeddingClient through)
- [src/panels/GemmaCodePanel.ts](../../../../src/panels/GemmaCodePanel.ts) (passes the cache into the subsystem)
...
**Files**:
- [src/config/PromptBudget.ts](../../../../src/config/PromptBudget.ts) (new exports: `countTokens`, `heuristicTokenCount`, `disposeEncoder`, `getTokenCounterStats`, `resetTokenCounterStats`)
- [src/chat/CompactionStrategy.ts](../../../../src/chat/CompactionStrategy.ts) (delegates `_computeTokensForMessage` to `countTokens`)
- [src/chat/PromptBuilder.ts](../../../../src/chat/PromptBuilder.ts) (delegates internal `estimateTokens` to `countTokens`)
- [src/tools/AgentLoop.ts](../../../../src/tools/AgentLoop.ts) (delegates `estimateTokensForString` to `countTokens`)
- [src/extension.ts](../../../../src/extension.ts) (`deactivate()` now calls `disposeEncoder()`)
- [package.json](../../../../package.json) (`tiktoken: ^1.0.17` added to runtime dependencies)
...
- [tests/unit/config/PromptBudget.tiktoken.test.ts](../../../../tests/unit/config/PromptBudget.tiktoken.test.ts) -- 11 tests covering: heuristic correctness (chars/4, code multiplier, empty input), `countTokens` non-empty/empty handling, ratio sanity check vs. heuristic on English text (within factor of 3), telemetry recording (load attempt, per-call counter increments), graceful degradation when tiktoken cannot load, `disposeEncoder` idempotence.
...
A  docs/v0.5.0/development/history/2026-04_phase-5-semantic-recall-and-budgeting.md
M  docs/v0.5.0/plans/implementation-plan.md  (Phase 5 exit checklist)
```

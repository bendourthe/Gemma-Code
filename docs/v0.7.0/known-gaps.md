...
| 10.O.19 | v0.7.1 hotfix | docs/v0.7.0/known-gaps.md item 2.1 resolution | BG | transferred to v0.8.0 plan (Phase 0 close-out) | Carried to v0.8.0; close when the `marked` v12 render perf regression is either fixed (renderer parity restored) or accepted with a documented trade-off, AND the `--exclude` rule is removed from the nightly bench gate. |

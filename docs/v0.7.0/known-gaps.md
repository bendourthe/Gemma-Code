...
| 10.O.12 | Phase 7 | docs/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.2 | MT | transferred to v0.8.0 plan (Phase 0.11) | Carried to v0.8.0; close when the background-workers end-to-end integration test lands. |
...
| 10.O.17 | Phase 8 | docs/v0.7.0/plans/v0.7.0-cycle.md sub-task 8.1 | NI | transferred to v0.8.0 plan (Phase 0.13) | Carried to v0.8.0; close when either the Python runner is explicitly canonised in the plan (ADR-0017) or the TS-native runner ships. |
| 10.O.18 | Phase 8 hotfix | docs/v0.7.0/plans/v0.7.0-cycle.md Phase 7 sub-task 7.1 | BG | transferred to v0.8.0 plan (Phase 0.8) | Carried to v0.8.0; close when the HNSW persist/reload root cause is identified and the env-gate (`HNSW_RUN_PERSIST=1`) is removed from the test. |
| 10.O.19 | v0.7.1 hotfix | docs/v0.7.0/known-gaps.md item 2.1 resolution | BG | transferred to v0.8.0 plan (Phase 0.9) | Carried to v0.8.0; close when the `marked` v12 render perf regression is either fixed (renderer parity restored) or accepted with a documented trade-off, AND the `--exclude` rule is removed from the nightly bench gate. |

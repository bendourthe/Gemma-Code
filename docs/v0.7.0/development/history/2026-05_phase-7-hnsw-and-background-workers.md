...
**Date**: 2026-05-14
**Plan reference**: [docs/v0.7.0/plans/v0.7.0-cycle.md](../../plans/v0.7.0-cycle.md) Phase 7
**Comparison reference**: [docs/v0.7.0/comparison-multi-source.md](../../comparison-multi-source.md) C32, C34
...
[src/storage/MemoryHnswIndex.ts](../../../../src/storage/MemoryHnswIndex.ts) is a new module that encapsulates the entire optional-native-dependency surface. The constructor is private; callers go through `MemoryHnswIndex.tryCreate(opts)` which returns `null` on any failure:
...
- [src/config/settings.ts](../../../../src/config/settings.ts): new `memoryHnswThreshold: number` field (default 1000, clamped to [0, 1_000_000]).
- [package.json](../../../../package.json) configuration: new `gemma-code.memoryHnswThreshold` schema entry. New `optionalDependencies` block: `"hnswlib-node": "^3.0.0"`.
- [src/storage/MemorySubsystem.ts](../../../../src/storage/MemorySubsystem.ts): new `hnsw?: { indexPath; threshold? }` option in `MemorySubsystemOptions`, threaded into `new MemoryStore(sharedDb, embedder, hnsw)`.
- [src/panels/ChatPanelInit.ts](../../../../src/panels/ChatPanelInit.ts): `buildMemorySubsystem` sets `hnsw.indexPath = path.join(globalStorageUri.fsPath, "memory.hnsw")` when `memoryHnswThreshold > 0`.
...
- [src/agents/types.ts](../../../../src/agents/types.ts): the `SubAgentType` union gains `"audit-worker" | "testgaps-worker"`.
- [src/agents/BackgroundWorkers.ts](../../../../src/agents/BackgroundWorkers.ts): pure functions `runAuditWorker(modifiedFiles, opts)` and `runTestgapsWorker(modifiedFiles, opts)` plus stand-alone parsers `parseGemmaCheckJson`, `formatAuditFindings`, `formatTestgapsOutput`. The `WorkerCommandRunner` type is the spawn-shaped function the workers call; the default runner wraps `child_process.spawn` with stderr / exitCode capture. Tests inject a fake runner via `SubAgentManager.setWorkerRunner` so no real process spawns during the unit suite.
- [src/agents/SubAgentManager.ts](../../../../src/agents/SubAgentManager.ts): `_runWorker(config, postMessage, spanId)` is the deterministic dispatch path. The early branch in `run` checks `config.type === "audit-worker" || config.type === "testgaps-worker"` BEFORE PromptBuilder / AgentLoop construction. The trace span is closed inside `_runWorker` so the worker shows up alongside verification/research in the trace dashboard.
...
The AgentLoop trigger refactor in [src/tools/AgentLoop.ts](../../../../src/tools/AgentLoop.ts):
...
- [src/panels/messages.ts](../../../../src/panels/messages.ts): `SubAgentStatusMessage.agentType` widened to include `"audit-worker" | "testgaps-worker"`.
- [src/panels/webview/runtime.ts](../../../../src/panels/webview/runtime.ts): the `subAgentStatus` label map gains `'audit-worker': 'Audit'` and `'testgaps-worker': 'Test Gaps'`.
...
Three new entries in [docs/v0.7.0/known-gaps.md](../../known-gaps.md) Section 10:
...
- `docs/v0.7.0/development/history/2026-05_phase-7-hnsw-and-background-workers.md` (this file).
...
- `docs/v0.7.0/known-gaps.md` -- three new in-cycle gap rows + summary recomputation.

...
3. **[PARTIAL 2026-07-16]** The v1.7.0 `SkillOptimizer` and `CandidateFrontier` are reachable from `nexus skills optimize` and `nexus skills frontier`, both built on the previously-missing production composition roots ([HeadlessOptimizerFactory.ts](../../../../modules/coding/skilloptimizer/HeadlessOptimizerFactory.ts): `createHeadlessSkillOptimizer` + `createHeadlessCandidateFrontier`), with the held-out gate + rejected-edit buffer + **human approval before any skill file is overwritten / any branch is promoted** carried intact (dry-run default deny gate; `--apply` prompts; `--apply --yes` for automation). Resolves the v1.7 `RT.P7.C` / `SO003.P3.D` / `SO005.P4.C` "no composition root / no CLI" gaps. DEFERRED (documented): only the desktop-sidecar method (`EM.P2.A`) -- needs a multi-call approval protocol + an app approval UI over the sidecar's one-shot request/response transport, best built + verified with the running desktop app.
...
| 2 | Surface the v1.7.0 skill optimizer (L1) -- sidecar wiring + `nexus skills optimize` / `nexus skills frontier` CLI | **CLI complete** (2026-07-16): the production composition roots + both `nexus skills optimize` and `nexus skills frontier` landed with the approval guardrails intact; only the desktop-sidecar method (`EM.P2.A`) deferred | Strong reasoning tier, high effort -- `claude-opus-4-8`, high (plan said `claude-sonnet-5`/medium; stayed up given the autonomous-skill-overwrite guardrail-carry sensitivity -- no-degradation) |

...
3. **[PARTIAL 2026-07-16]** The v1.7.0 `SkillOptimizer` is reachable from a new `nexus skills optimize` CLI, built on the previously-missing production composition root ([HeadlessOptimizerFactory.ts](../../../../modules/coding/skilloptimizer/HeadlessOptimizerFactory.ts)), with the held-out gate + rejected-edit buffer + **human approval before any skill file is overwritten** carried intact (dry-run default deny gate; `--apply` prompts per edit via readline; `--apply --yes` for automation). DEFERRED (documented, forward-tier): `nexus skills frontier` (needs the `CandidateFrontier` composition root + git-worktree wiring -- `EM.P2.B`) and the desktop-sidecar method (needs a multi-call approval protocol + an app approval UI over the sidecar's one-shot request/response transport -- `EM.P2.A`). This resolves the v1.7 `RT.P7.C` / `SO003.P3.D` "no composition root / no CLI" gap for the optimizer.
...
| 2 | Surface the v1.7.0 skill optimizer (L1) -- sidecar wiring + `nexus skills optimize` / `nexus skills frontier` CLI | **partial** (closed 2026-07-16): the production composition root + `nexus skills optimize` landed with the approval guardrails intact; `nexus skills frontier` (`EM.P2.B`) + the desktop-sidecar method (`EM.P2.A`) deferred | Strong reasoning tier, high effort -- `claude-opus-4-8`, high (plan said `claude-sonnet-5`/medium; stayed up given the autonomous-skill-overwrite guardrail-carry sensitivity -- no-degradation) |
...
**Status**: PARTIAL 2026-07-16. Landed: the previously-missing production composition root [HeadlessOptimizerFactory.ts](../../../../modules/coding/skilloptimizer/HeadlessOptimizerFactory.ts) (`createHeadlessSkillOptimizer` assembles a runnable `SkillOptimizer` from the shipped v1.7 seams over an injected LLM port -- the `RT.P7.C`/`SO003.P3.D` gap) and the `nexus skills optimize <id> [--apply] [--yes] [--model] [--max-rounds]` CLI ([bin/nexus.mjs](../../../../bin)), carrying the held-out gate + rejected-edit buffer + human-approval-before-overwrite intact (deny-all gate in the default dry-run; a readline prompt per edit under `--apply`; approve-all only under `--apply --yes`). Verified: 17 new tests (factory integration + CLI parseArgs); `tsc -b` clean; lint 0 warnings; `check-architecture` 0 errors; full suite green (4617 passed / 6 skipped / 0 failed); CLI no-id path smoke exits 2. DEFERRED to forward-tier follow-ups: `nexus skills frontier` (`EM.P2.B` -- the `CandidateFrontier` composition root + git-worktree wiring) and the desktop-sidecar method (`EM.P2.A` -- needs a multi-call approval protocol + app approval UI, since the sidecar transport is one-shot request/response with no server-push channel). Deferring the desktop UI preserves the human-approval guardrail rather than shipping a half-built approval path.

### Sub-tasks

#### 2.1 -- L1a: desktop-sidecar wiring

- [ ] EM004 Expose the `SkillOptimizer` + `CandidateFrontier` through a JSON-RPC method on the Node sidecar so the desktop app can launch an optimization run and review/approve proposed skill edits -- **DEFERRED (`EM.P2.A`)**: the sidecar transport is one-shot request/response with no server-push channel (verified in the pre-implementation review), so the interactive human-approval round-trip requires a multi-call protocol (start -> pending-approval token -> approve) plus a React approval UI. Deferred to preserve the guardrail rather than ship a half-built approval path; the composition root ([HeadlessOptimizerFactory.ts](../../../../modules/coding/skilloptimizer/HeadlessOptimizerFactory.ts)) it would reuse now exists.
...
- [x] EM005 Add `nexus skills optimize` (+ `nexus skills frontier`) CLI commands over the headless seams, with the same approval + held-out + buffer guardrails -- **`optimize` done 2026-07-16**: the composition root [HeadlessOptimizerFactory.ts](../../../../modules/coding/skilloptimizer/HeadlessOptimizerFactory.ts) + `runSkillsOptimize` in [bin/nexus.mjs](../../../../bin) (dry-run default deny gate; `--apply` readline prompt; `--apply --yes` auto-approve; `--model`/`--max-rounds`; loads the live catalog + optimizer-visible golden splits). `nexus skills frontier` DEFERRED (`EM.P2.B`): needs the `CandidateFrontier` composition root + the `WorktreeCandidateManager` git wiring + candidate-diagnosis seeding; the `optimize` path is the headline bounded-edit loop and lands first.
...
- [x] EM006 Tests for the optimize surface -- **done 2026-07-16**: [headless-optimizer-factory.test.ts](../../../../tests/integration/skilloptimizer/headless-optimizer-factory.test.ts) (3: the factory assembles + runs + writes nothing under the deny gate on a passing train split; the terminal gates; the `withCritic` branch) + [nexus-cli.test.ts](../../../../tests/unit/cli/nexus-cli.test.ts) parseArgs cases for `skills optimize` (2). They run in the existing suites (already on CI); no CI change needed. Sidecar/frontier surface tests land with `EM.P2.A`/`EM.P2.B`.

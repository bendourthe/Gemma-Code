...
**Date**: 2026-05-16
**Plan reference**: [docs/v0.8.0/plans/v0.8.0-cycle.md](../../plans/v0.8.0-cycle.md) -- Phase 7
...
- `docs/v0.7.0/plans/v0.7.0-cycle.md` -- rewrote the provisional ADR numbers across sub-tasks 3.8, 4.8, 5.3, and the Phase-table summary row: ADR-0006 -> 0012 (compress tool), ADR-0008 -> 0013 (webview render protocol), ADR-0007 -> 0014 (memory file architecture). Each updated line includes an inline "v0.8.0 Phase 7.1 fix" annotation so the rewrite is auditable.
- `docs/v0.7.0/known-gaps.md` -- moved 10.O.4, 10.O.8, 10.O.9, 10.O.10 from "transferred" status to fully `Resolved in v0.8.0 Phase 7.{1,A,B,C}` with one-line completion notes citing the exact files and tests.
- `docs/v0.8.0/known-gaps.md` -- appended five new Phase 7 entries (10.O.AA mutation, 10.O.BB live-Ollama capture, 10.O.CC pen-test, 10.O.DD GitHub release, 10.O.EE cycle-wide changelog narrative), moved the four resolved v0.7.0 carryovers to the Resolved table, and recomputed the summary table (31 open / 14 resolved).
...
4. **CHANGELOG.md not manually edited**. semantic-release rewrites `CHANGELOG.md` on every `feat()` / `fix()` push to `main`, so a manual edit would be clobbered on the next release commit. The cycle-wide v0.8.0 narrative is preserved in the per-phase history files (`docs/v0.8.0/development/history/phase-0N.md`) and in the v0.8.0 `known-gaps.md` Summary rows. An optional consolidated `docs/v0.8.0/CHANGELOG-CYCLE.md` is tracked as 10.O.EE.
...
- 10.O.CC (DF / P1) -- full pen-test re-run against the v0.8.0 source (Attack Paths A / B / C from `docs/v0.6.0/review/penetration-test.md`).
...
1. On a quiescent dev workstation with `ollama serve` running and `gemma4:e4b` pulled, execute:
   ...
   - `scripts/check-bench-regressions.mjs` against v0.7.0 and v0.6.0; record deltas in `docs/v0.8.0/performance-baselines.md`.
2. On a Linux x64 host (or after 10.O.D's vitest 1.6.1 upgrade), run `npm run mutate` and update `docs/v0.8.0/review/mutation-report.md`.
3. Walk pen-test Attack Paths A / B / C against the v0.8.0 source; document findings in `docs/v0.8.0/review/penetration-test.md`. P0 / P1 findings block the release tag.

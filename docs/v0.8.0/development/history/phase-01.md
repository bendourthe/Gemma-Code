...
**Date**: 2026-05-15
**Plan reference**: [docs/v0.8.0/plans/v0.8.0-cycle.md](../../plans/v0.8.0-cycle.md) Phase 1
**Status**: complete
...
- Created [src/chat/prompts/compaction.md](../../../../src/chat/prompts/compaction.md) carrying the canonical prefix copy.
- Exported `COMPACTION_SUMMARY_PREFIX` from [src/chat/CompactionStrategy.ts](../../../../src/chat/CompactionStrategy.ts:13) and wired it into the `LlmSummary.apply()` summary message between the `[Conversation summary]` header and the LLM-generated body.
- Added a regression test in [tests/unit/chat/CompactionStrategy.test.ts](../../../../tests/unit/chat/CompactionStrategy.test.ts) asserting the prefix is present in the rendered summary.
...
- Added `PLAN_DENIAL_TEMPLATE` and `buildDenialMessage(feedback)` exports in [src/chat/PlanMode.ts](../../../../src/chat/PlanMode.ts).
...
- Added a `PlanDenyMessage` shape to [src/panels/messages.ts](../../../../src/panels/messages.ts) and a `planDeny` case to [src/panels/ChatMessageRouter.ts](../../../../src/panels/ChatMessageRouter.ts) that injects the rendered template as a system message.
...
- Created [src/chat/prompts/planModeCapabilities.md](../../../../src/chat/prompts/planModeCapabilities.md) with the canonical render-primitive reference.
- Exported `PLAN_MODE_CAPABILITIES_REMINDER` from `PlanMode.ts` (const, no fs at hot path).
- [src/chat/PromptBuilder.ts](../../../../src/chat/PromptBuilder.ts) now concatenates `PLAN_MODE_SYSTEM_ADDENDUM` and `PLAN_MODE_CAPABILITIES_REMINDER` when `planModeActive` is true, with `estimatedTokens` recomputed against the combined content.
...
- [src/skills/catalog/lens/SKILL.md](../../../../src/skills/catalog/lens/SKILL.md) -- analytical-lens-first answer pattern.
- [src/skills/catalog/incident-commander/SKILL.md](../../../../src/skills/catalog/incident-commander/SKILL.md) -- triage / classify / 3-tier remediation / verify / playbook.
- [src/skills/catalog/council/SKILL.md](../../../../src/skills/catalog/council/SKILL.md) -- advocate / architect / user-impact / synthesis. Latency cost flagged in the SKILL.md body since this skill performs three inference passes.
...
- Updated [tests/integration/commands/skill-execution.test.ts](../../../../tests/integration/commands/skill-execution.test.ts) catalog-count assertion from 13 to 16 and added the three names to the `expected` list.

...
**Date**: 2026-05-16
**Plan reference**: [docs/v0.8.0/plans/v0.8.0-cycle.md](../../plans/v0.8.0-cycle.md) Phase 3
**Status**: complete
...
- Added [tests/unit/panels/webview/render/quickLabels.test.ts](../../../../tests/unit/panels/webview/render/quickLabels.test.ts) (16 cases covering catalog shape, lookup, custom-overlay loader, malformed-row filtering, render output, hover-title attribute, and click dispatch).

### 3.4 -- Improvement-hook file (item B7)

- Created [src/chat/ImprovementHook.ts](../../../../src/chat/ImprovementHook.ts) with `hookFilePath(name, rootDir?)`, `loadHook(name, rootDir?)`, and `renderHookAsSystemMessage(name, rootDir?)`. The `HookName` type is the authoritative list of recognised hook names; Phase 3 ships only `enterplanmode-improve`. Empty / whitespace-only / missing files return `null`; read errors other than `ENOENT` log via `getLogger().warn` and return `null` so the plan-mode entry path keeps flowing.
- Wired into [src/panels/ChatMessageRouter.ts](../../../../src/panels/ChatMessageRouter.ts) `_handleSetEditMode`: when plan mode activates, the router rebuilds the system prompt (so the built-in addendum + PFM reminder fire first) and then -- if the hook file has non-empty content -- appends a `## User-supplied plan-mode rules` system message with the user's overlay.
- Added a new VS Code command `gemma-code.hooks.editPlanModeHook` in [src/extension.ts](../../../../src/extension.ts) that opens the file (and lazily seeds it with a starter template) in a tab. Registered in [package.json](../../../../package.json) under the `Gemma Code: Edit Plan-Mode Improvement Hook` title.
- Documented the file format, safety boundary, and example bodies in [docs/v0.8.0/improvement-hooks.md](../../improvement-hooks.md).
...
- Added [tests/unit/chat/ImprovementHook.test.ts](../../../../tests/unit/chat/ImprovementHook.test.ts) (5 cases: path build, missing file, empty body, populated body trim, render-as-system-message).

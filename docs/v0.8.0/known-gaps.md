...
**Last updated**: 2026-05-16 (Phase 5 close).
...
| 10.O.O | Phase 5 | docs/v0.8.0/plans/v0.8.0-cycle.md sub-task 5.9 | WN | P2 | `npm run check:prompts` reports 38 errors + 4 warnings on the existing `src/skills/catalog/**/SKILL.md` tree -- predominantly inline em-dashes carried over from the v0.7.0 catalog. The rule infrastructure is correct (the unit tests under `tests/unit/lib/checks-prompt-rules.test.ts` cover positive + negative cases); the latent encoding violations were not touched in Phase 5 because doing so would have been an out-of-scope clean-up sweep. | Phase 7 polish (or a dedicated sub-task between Phase 6 and Phase 7) should run `npm run check:prompts -- --json` and apply the ASCII-only fixes across the catalog. Until then the linter ships with documented findings and CI does not gate on `check:prompts` exit code. |
